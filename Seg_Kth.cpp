...
ll findkth(ll i,ll l,ll r, ll k){
    if(l==r){
        return l;
    }
    ll mid = (r-l)/2 + l;
    ll leftcnt=segTree[2*i+1];
    if(k<=leftcnt){
        return findkth(2*i+1,l,mid,k);
    }
    else{
        return findkth(2*i+2,mid+1,r,k-leftcnt);
    }
}
...
void solve(){
    ...
    while(q--){
        cin>>x;
        ll idx=findkth(0,0,n-1,x);
        cout<<v[idx]<<" ";
        updateTree(0,idx,0,n-1,0);
